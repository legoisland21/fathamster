...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd) {
	...
	Texture2D player_ = LoadTexture("textures/robin2.png");
	Texture2D floor_ = LoadTexture("textures/stuff2.jpg");
	InitAudioDevice();
	...
	while (!WindowShouldClose()) {
		...
		if (!flop) {
			...
			DrawCircle(50, 50, 50, BEIGE);
		}
		float groundOffsetX = fmod(camera.target.x, floor.width);
		float groundOffsetX2 = fmod(camera.target.x, floor_.width);
		if (!flop) {
			...
			for (int j = -10; j < screen.x / floor_.width + 70; j++) {
				DrawTexture(floor_, (int)(j * floor_.width - groundOffsetX2), 650, WHITE);
			}
		}
		...
		if (!flop) {
			...
			DrawTextureEx(player_, position, 0.0f, 0.1f, WHITE);
		}
		EndMode2D();
		EndDrawing();
		// STOP DRAWING
