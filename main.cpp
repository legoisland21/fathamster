...
string LowerCase(string& str) {
    transform(str.begin(), str.end(), str.begin(),
        [](unsigned char c) { return tolower(c); }); // Transform to lowercase
    return str;
}

void HandleCheats() {
    CodeUi = tinyfd_inputBox("Cheatcode", "Enter your desired Cheat code:", ""); // Open dialog
    if (CodeUi == NULL || CodeUi[0] == '\0') return; // Check if the dialog is NULL/empty
    string BasicCode(CodeUi);
    string Code = LowerCase(BasicCode); // Make the code lowercase
...
int main() {
    ...
    if(Sky.id == 0 || Ground.id == 0 || Player.id == 0 || PlayerJump.id == 0) { tinyfd_messageBox("ERROR", "Error: Textures not loaded, game will quit now." , "ok", "error", 1); return -1; } // Texture check
    InitAudioDevice();
    ...
    if(CheatOn.frameCount == 0 || CheatOff.frameCount == 0 ) { tinyfd_messageBox("ERROR", "Error: Sounds not loaded, game will quit now." , "ok", "error", 1); return -1; } // SFX Check
    if(BGM.ctxData == NULL) { tinyfd_messageBox("ERROR", "Error: Music not loaded, game will quit now." , "ok", "error", 1); return -1; } // BGM check
    PlayMusicStream(BGM);
    SetMusicVolume(BGM, 0.15f);

    Vector2 PlayerPos = {0, ScreenHeight - GroundLevel}; // Set player position
    ...
    while(!WindowShouldClose()) {
        ...
        if(IsKeyPressed(KEY_APOSTROPHE) && DevMode)  HandleCheats();
        ...
        if (!Jumping && !InfJumping) { DrawTexturePro(Player, SourceRect, DestRect, {0,0}, 0.0f, WHITE); } // Draw normal animation
        else if (Jumping || InfJumping) { DrawTexturePro(PlayerJump, SourceRect, DestRect, {0,0}, 0.0f, WHITE); } // Draw jumping animation
        FPS = GetFPS();
        if (FPS > MFPS) MFPS = FPS; // Get max FPS
        if(ShowFps) {
            ...
            DrawText(TextFormat("X Position: %d", (int)PlayerPos.x), 0, 30, 30, BLACK);
            DrawText(TextFormat("Y Position: %d", (int)PlayerPos.y), 250, 30, 30, BLACK);
            DrawText(TextFormat("Mouse X: %d", (int)MousePos.x), 0, 60, 30, BLACK);
            DrawText(TextFormat("Mouse Y: %d", (int)MousePos.y), 250, 60, 30, BLACK);
