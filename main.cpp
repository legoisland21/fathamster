...
bool InfJump, UnlockFps, ShowFps, MoonJump, FloppaMode, DevMode;
int TempGravity;
...
int main() {
    ...
    while(!WindowShouldClose()) {
        ...
        if(IsKeyPressed(KEY_APOSTROPHE) && DevMode) {
            ...
            if (CodeUi == NULL || CodeUi[0] == '\0') { tinyfd_messageBox("Error","Enter a valid code. The program will now quit.", "ok", "warning", 1); return -1; }
            string Code(CodeUi);
            ...
            else if (Code == "ChangeFPS") { FPSUi = tinyfd_inputBox("FPS cheat code", "How much FPS?:", ""); LIMITFPS = stoi(FPSUi); SetTargetFPS(LIMITFPS); }
            else if (Code == "Moon") { MoonJump = !MoonJump; MoonUi = tinyfd_inputBox("Moon cheat code", "What gravity? (default 0.5f):", ""); TempGravity = stof(MoonUi); Gravity = TempGravity; }
        }
        ...
        for (int i = 0; i != 7; i++) {
            DrawTextureEx(Ground, {0 + (i * 250.0f), screenHeight - groundLevel + 100}, 0.0f, 0.15f, WHITE);
        }
